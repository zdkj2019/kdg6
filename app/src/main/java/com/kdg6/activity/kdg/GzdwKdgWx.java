 ...
 *
 */
public class GzdwKdgWx extends FrameActivity {
	...
	private String flag, zbh, message,dlbm,zlbm,xlbm,zsdbm;
	private String[] from;
	private int[] to;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		appendMainBody(R.layout.activity_kdg_gzdw);
		initVariable();
		initView();
		initListeners();
		showProgressDialog();
	...
	protected void initListeners() {
			...
			public void onClick(View v) {
				switch (v.getId()) {
					...
					case R.id.cancel:
						...
						String zlmc = ((Map<String, String>) spinner_gzzl.getSelectedItem()).get("name").trim();
						String xlmc = ((Map<String, String>) spinner_gzxl.getSelectedItem()).get("name").trim();
						if ("".equals(zlbm)) {
							toastShowMessage("请选择故障模块！");
							return;
						}
						Intent intent = new Intent(getApplicationContext(),GzzskList.class);
						intent.putExtra("zlbm", zlbm);
						intent.putExtra("xlmc", xlmc);
						startActivityForResult(intent, 1);
						break;
					case R.id.confirm:
						xlbm = ((Map<String, String>) spinner_gzxl.getSelectedItem()).get("id").trim();
						xlmc = ((Map<String, String>) spinner_gzxl.getSelectedItem()).get("name").trim();
						if ("".equals(xlbm)) {
							toastShowMessage("请选择故障原因！");
							return;
						}
						dialogShowMessage("确认故障原因是："+xlmc+",确认后不可修改!",new DialogInterface.OnClickListener() {
		...
		OnItemSelectedListener onItemSelectedListener_gzdl = new AdapterView.OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1,
				...
				SimpleAdapter adapter = new SimpleAdapter(GzdwKdgWx.this,
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == 1 && resultCode == Activity.RESULT_OK) {
			...
			zsdbm= data.getStringExtra("zsdbm");
	...
	protected void getWebService(String s) {
		...
		if(s.equals("querygzxl")){
			try {
				JSONObject jsonObject = callWebserviceImp.getWebServerInfo(
						"_PAD_YWCX_ZSDNR_GZYY", zsdbm, "uf_json_getdata", this);
				flag = jsonObject.getString("flag");
				if (Integer.parseInt(flag) > 0) {
					JSONArray jsonArray = jsonObject.getJSONArray("tableA");
					for (int i = 0; i < jsonArray.length(); i++) {
						JSONObject temp = jsonArray.getJSONObject(i);
						Map<String, String> item = new HashMap<String, String>();
						String id = temp.getString("gdxl");
						String sjbm = temp.getString("gdxl_mc");
						xlbm = id;
					}
					Message msg = new Message();
	...
	private Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			...
			switch (msg.what) {
				...
				case Constant.NUM_7:
					for (int i = 0; i < gzbm_3_list.size(); i++) {
						Map<String, String> map = gzbm_3_list.get(i);
						if (xlbm.equals(map.get("id"))) {
							spinner_gzxl.setSelection(i);
							xlbm = null;
							break;
