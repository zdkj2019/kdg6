...
public class PjfkMxActivity extends FrameActivity {
	...
	protected void initListeners() {
		...
		edit_ckkfmc.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				chooseType = "ckkfmc";
				Intent intent = new Intent(PjfkMxActivity.this,
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == 1 && resultCode == 1 && data != null) {
			...
			if ("ckkfmc".equals(chooseType)) {
				edit_ckkfmc.setTag(map.get("id"));
				edit_ckkfmc.setText(map.get("name"));
			} else if ("rkkfmc".equals(chooseType)) {
				edit_rkkfmc.setTag(map.get("id"));
				edit_rkkfmc.setText(map.get("name"));
			} else if ("ckcwmc".equals(chooseType)) {
				edit_ckcwmc.setTag(map.get("id"));
				edit_ckcwmc.setText(map.get("name"));
			} else if ("rkcwmc".equals(chooseType)) {
				edit_rkcwmc.setTag(map.get("id"));
				edit_rkcwmc.setText(map.get("name"));
			} else if ("hpmc".equals(chooseType)) {
				edit_hpmc.setText(map.get("name"));
				edit_hpmc.setTag(map.get("id"));
				for (int i = 0; i < data_hp.size(); i++) {
					Map<String, String> temp = data_hp.get(i);
					if (map.get("id").equals(temp.get("id"))) {
	...
	protected void getWebService(String s) {
		...
		if ("query".equals(s)) {
			try {
				JSONObject jsonObject_kfrk = callWebserviceImp
						.getWebServerInfo("_PAD_KFFK_RKFWCX", DataCache.getinition().getUserId(),
			...
			try {
				JSONObject jsonObject_cw = callWebserviceImp.getWebServerInfo(
						"_PAD_KFCK_CW", DataCache.getinition().getUserId()
								+ "*" + currkfck+"*2", "uf_json_getdata", this);
		...
		if ("getcwrk".equals(s)) {
			try {
				JSONObject jsonObject_cw = callWebserviceImp.getWebServerInfo(
						"_PAD_KFFK_RKFWCX2", DataCache.getinition().getUserId()+"*"+currkfrk, "uf_json_getdata", this);
				flag = jsonObject_cw.getString("flag");
				if (Integer.parseInt(flag) > 0) {
					JSONArray array = jsonObject_cw.getJSONArray("tableA");
					data_cwrk = new ArrayList<Map<String,String>>();
			...
			try {
				JSONObject jsonObject_hp = callWebserviceImp.getWebServerInfo(
						"_PAD_DBCKHPXX", DataCache.getinition().getUserId()
								+ "*" + currcw+ "*" + currkfck, "uf_json_getdata", this);
