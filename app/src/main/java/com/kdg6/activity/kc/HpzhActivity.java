...
public class HpzhActivity extends FrameActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		appendMainBody(R.layout.activity_hpzh);
		initVariable();
		initView();
		initListeners();
	...
	protected void initListeners() {
		...
		confirm.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if(!isNotNull(edit_sgdh)){
					toastShowMessage("工单号不能为空！");
					return;
				}
				if(data_mx.size()==0){
					toastShowMessage("转换明细不能为空！");
					return;
				}
				showProgressDialog();
		...
		btn_addmx.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				Intent intent = new Intent(HpzhActivity.this,HpzhMxActivity.class);
