...
public class HpzhMxActivity extends FrameActivity {
	...
	protected void initListeners() {
		...
		edit_ckkfmc.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				chooseType = "ckkfmc";
				Intent intent = new Intent(HpzhMxActivity.this,
		...
		if ("query".equals(s)) {
			try {
				JSONObject jsonObject_kfrk = callWebserviceImp
						.getWebServerInfo("_PAD_KFCRK_KF", DataCache
		...
		if ("getcwck".equals(s)) {
			try {
				JSONObject jsonObject_cw = callWebserviceImp.getWebServerInfo(
						"_PAD_KFCRK_CW", DataCache.getinition().getUserId()
